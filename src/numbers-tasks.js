...
function getAverage(value1, value2) {
  ...
  return sumValue > Number.MAX_VALUE ? Number.MAX_VALUE : sumValue / 2;
}
...
function getLastDigit(value) {
  ...
  // return valueArr.length === 1 ? +valueArr[0] : +valueArr[valueArr.length - 1];
  return value % 10;
}
...
function toNumber(value, def) {
  ...
  return Number.isNaN(parseIntValue) ? def : parseIntValue;
}
...
function getCountOfOddNumbers(number) {
  const positiveFormat = Math.abs(number);
  const oddNumbers = [];
  for (let i = 0; i <= positiveFormat; i += 1) {
    if (i % 2 !== 0) {
      oddNumbers.push(i);
    }
  }
